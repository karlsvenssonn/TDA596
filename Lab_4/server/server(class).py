...
class Server:
    def __init__(self, host, port, node_list, node_id):
        ...
        if self.node_id == 1:
            ...
            byzantine_id = random.randint(1, args.nbv + 1)
            print str(byzantine_id)
            thread = Thread(target = self.byzantine_select_init, args = byzantine_id)
            thread.daemon = True
            thread.start()

    def _route(self):
        self._app.route('/', method="GET", callback=self.index)
        self._app.route('/vote/result', method="GET", callback=self.get_vote)
        self._app.route('/vote/<vote_type>', method="POST", callback=self.client_add_received)
        ...
        self._app.route('/byzantine_select/<byzantine_id:int>', method="POST", callback=self.byzantine_select)
    ...
    # POST '/board'
    def client_add_received(self, vote_type):
    ...
    def propagation_received(self, action, element_id):
        ...
        if action == 'ADD':
            ...
            self.delete_element_from_store(element_id)
    # POST '/byzantine_select'        
    def byzantine_select_init(self, byzantine_id):
        if self.node_id == byzantine_id:
            print "I am byzantine"
        else:
            self.propagate_to_nodes('/byzantine_select/' + str(byzantine_id))

    def byzantine_select(self, byzantine_id):
        if self.node_id == byzantine_id:
            print "I am byzantine"
